...
def normalize(loss, weight):
    ...
    num_weight = torch.clamp(weight_sum, 1.0)
    loss = (loss * weight).sum(dim=0) / num_weight
    return loss


def b_accu(binary_cls_logits, binary_cls_targets, is_logits=True):
    return (b_pred(binary_cls_logits, is_logits=is_logits) == binary_cls_targets).type(
        torch.float
    )
...
def loss_maker(cls_logits, attn_logits, cls_targets, attn_targets, config):
    ...
    for l, t in zip(cls_logits, cls_targets):
        ...
        ls = normalize(
            ls,
            weighting_maker(
                t, target_type="ignore", ignore_value=config["ignore_value"]
            ),
        )
        loss_cls.append(ls)
    ...
    for l, t in zip(attn_logits, attn_targets):
        if version == 1:
            # tt = nn.functional.interpolate(t, l.shape[2:], mode="nearest")
            # l = nn.functional.interpolate(
            #     l, t.shape[2:], mode="bicubic", align_corners=True
            # )
            ...
            ls = normalize(
                ls,
                weighting_maker(
                    tt, target_type="ignore", ignore_value=config["ignore_value"]
                ),
            )
...
def accuracy_maker(cls_logits, attn_logits, cls_targets, attn_targets, config):
    ...
    for l, t in zip(attn_logits, attn_targets):
        # t = nn.functional.interpolate(t, l.shape[2:], mode="nearest")
        l = l.reshape(-1, 1)
        t = t.view(-1, 1)
        au = b_accu(l, t)
...
def prediction_maker(cls_logits, cls_targets, config, is_logits=True):
    pred_cls = list()
    prob_cls = cls_logits
    for l in cls_logits:
        _, p = pred(l)
