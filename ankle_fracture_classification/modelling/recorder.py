...
class Recorder:
    ...
    def add_info(self, epoch_no, tra_val_tag, new_info):
        ...
        for k in new_info:
            ...
            if type(v) == torch.Tensor:
                ...
                if len(v.shape) >= 2:
                    if (
                        np.prod(v.shape) / v.shape[0] > 1000
                        and np.mod(epoch_no + 1, 5) != 0
                    ):
        ...
        for k in keys:
            for p_tag in plot_tag:
                if k[: len(p_tag)] == p_tag:
                    ...
                    for tra_val_tag, line_style, color_style in zip(
                        ["tra", "val"], ["-", "--"], ["g", "r"]
                    ):
                        # batch_size = [self.master_dict[e][tra_val_tag]['batch_size'] for e in self.master_dict]
                        batch_size = [
                            self.master_dict[e][tra_val_tag]["nvac" + k[len(p_tag) :]]
                            for e in self.master_dict
                        ]
                        normalizer = [np.maximum(b.cumsum(), 1.0) for b in batch_size]
                        values = [
                            self.master_dict[e][tra_val_tag][k]
                            for e in self.master_dict
                        ]
                        values_cumsum = [
                            (v * b).cumsum() for v, b in zip(values, batch_size)
                        ]
    ...
    def master_dict_to_json(self, path):
        with open(os.path.join(path, "master_dict.json"), "w") as f:
            json.dump(self.master_dict, f, indent=4, sort_keys=True, cls=NumpyEncoder)
