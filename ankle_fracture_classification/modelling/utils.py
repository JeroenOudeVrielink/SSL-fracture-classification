...
# from coco.non_max_suppression import non_max_suppression
...
def remove_named_children(model, named_children_to_remove):
    new_model = nn.Sequential()
    for name, child in model.named_children():
        if name not in named_children_to_remove:
...
def set_learning_rate(optimizer, epoch_no, args):
    if args.lr_decay_policy == "exp":
        ...
        lr_decay_rate = math.exp(
            (math.log(args.learning_rate_end) - math.log(args.learning_rate))
            / (args.num_epochs - 1)
        )
        epoch_lr = args.learning_rate * lr_decay_rate**epoch_no
    elif args.lr_decay_policy == "linear":
        lr_decay_rate = (args.learning_rate - args.learning_rate_end) / (
            args.num_epochs - 1
        )
        epoch_lr = args.learning_rate - lr_decay_rate * epoch_no
    else:
...
def augment_images(
    source_images, cams, labels, preds, force_resize=False, resize_size=224
):
    if force_resize:
        source_images = F.interpolate(
            source_images,
    ...
    for im_idx, sim in enumerate(source_images[:10]):
        ...
        for cl in range(min(att_maps.shape[0], 4)):
            ...
            p = preds[im_idx].item()
            disp_str = "L{}P{}".format(
                int(l) if l.is_integer() else "{:0.3f}".format(l),
            ...
            augmented = np.moveaxis(augmented, (0, 1, 2), (1, 2, 0)) / 255.0
...
def make_ann(
    ...
    # source_images = source_images.permute([0, 2, 3, 1]).numpy()

    # maps, seg_maps = maps.max(dim=1)
    seg_logits = seg_logits.detach()
    # seg_maps = seg_maps.detach().cpu().numpy()

    anns = list()
    for idx, img_info in enumerate(targets_info):
        # map = maps[idx, 1:]
        map = seg_logits[idx]
        size = all_proposal_sizes[idx]
        # source_image = source_images[idx]
        # proposals, _, boxes = get_selective_search_proposals(source_image)
        # proposals = torch.tensor(proposals).to(device=device)
        proposals = all_proposals[idx]
        boxes = all_boxes[idx]
        img_id = img_info[0]
        # h = coco.imgs[img_id]['height']
        # w = coco.imgs[img_id]['width']

        proposals_probs = map.unsqueeze(dim=1) * proposals.unsqueeze(dim=0)
        proposals_probs = proposals_probs.sum(dim=(2, 3)) / (
            size.unsqueeze(dim=0) + 1e-10
        )
        proposals_probs = torch.softmax(proposals_probs, dim=0)
        proposal_scores, proposals_preds = proposals_probs.max(dim=0)
        proposal_scores = proposal_scores.cpu().numpy()
        proposals_preds = proposals_preds.cpu().numpy()

        # picked_boxes = list()
        # picked_scores = list()
        # picked_preds = list()
        # for p in np.unique(proposals_preds):
        #     if p == 0:
        #         continue
        #     selection = (proposals_preds == p)
        #     cls_boxes = np.array(boxes)[selection]
        #     cls_scores = proposal_scores[selection]
        #     box, pick = non_max_suppression(boxes=cls_boxes, probs=cls_scores)
        #     picked_boxes.extend(box)
        #     picked_scores.extend(cls_scores[pick])
        #     picked_preds.extend(proposals_preds[selection][pick].tolist())

        selection = proposals_preds != 0
        if np.sum(selection) == 0:
            continue
        proposal_scores = proposal_scores[selection]
        proposal_preds = proposals_preds[selection]
        picked_boxes, pick = non_max_suppression(
            boxes=np.array(boxes)[selection], probs=proposal_scores
        )
        picked_scores = proposal_scores[pick]
        picked_preds = proposal_preds[pick]

        # img = (source_images[idx].permute([1, 2, 0]).numpy()*256.).astype('uint8')
        # for pred, score, box in zip(picked_preds, picked_scores, picked_boxes):
        #     img = cv2.rectangle(img, tuple(box[:2]), tuple(box[2:]), (0, 255, 0), 1)
        #     if type(img) != np.ndarray:
        #         img = img.get()
        #     img = cv2.putText(img, data_loader.dataset.cats_dict[pred], tuple(box[:2]),
        ...
        for pred, score, box in zip(picked_preds, picked_scores, picked_boxes):
            ...
            if cls_id not in data_loader.dataset.cats_dict:
                continue

            h = coco.imgs[img_id]["height"]
            w = coco.imgs[img_id]["width"]

            spatial_size = np.asarray(source_images.shape[2:])
            spatial_size = np.concatenate([spatial_size, spatial_size], axis=0)
            original_size = np.asarray([h, w, h, w])
            original_box = np.round(box / spatial_size * original_size).astype("int64")
            ann = {
                "image_id": img_id,
                "category_id": cls_id,
                "score": score,
            ...
            # utils.plot_image(coco.annToMask(ann), 2)
            anns.append(ann)
...
def get_mask(coco, ann_id):
    ...
    m = coco.annToMask(
        {"image_id": img_id, "segmentation": coco.annToRLE(coco.anns[ann_id])}
    )
    return m
...
def get_selective_search_proposals(img):
    ...
    _, regions = selectivesearch.selective_search(
        img, scale=500, sigma=0.9, min_size=25
    )
    ...
    num_proposals = 64
    regions = [
        regions[r_idxs[r]] for r in range(np.minimum(len(r_idxs), num_proposals))
    ]
...
def plotter(idx, sub_idx, image_list, name_list, rand_targets, data_loader):
...
def brush_map(color_seg_maps, att_orig):
    ...
    for seg_idx, m in enumerate(color_seg_maps):
        for i in range(1, int(torch.max(m).cpu().numpy())):
            ...
            binary_mask = binary.view(-1, binary.shape[0], binary.shape[1]).type(
                torch.float
            )
...
def brush_map2(color_seg_maps, att1, att2=None):
    ...
    for i in range(1, int(torch.max(color_seg_maps).item()) + 1):
        ...
        binary_mask = binary.view(-1, 1, binary.shape[1], binary.shape[2]).type(
            torch.float
        )

        selection = binary_mask * att1
        avg_value = selection.sum(dim=(2, 3), keepdim=True) / num_pixels.view(
            -1, 1, 1, 1
        )
        ...
        if att2 is not None:
            ...
            avg_value2 = selection2.sum(dim=(2, 3), keepdim=True) / num_pixels.view(
                -1, 1, 1, 1
            )
...
def grayscale_grad(rgb_grad):
    ...
    if rgb_grad.shape[1] != 3:
        raise ValueError(
            "The input tensor should be in batch_size x (channel=3) x h x w format"
        )
