...
import time
import datetime
from modelling.train_utils import (
    loss_maker,
    accuracy_maker,
    prediction_maker,
    target_maker,
...
class Trainer:
    ...
    def train(self):
        for epoch_no in range(self.args.num_epochs):
            ...
            # self.schedule.step()
            self.schedule(self.optimizer, epoch_no, self.args)
            ...
            if (
                np.mod(disp_epoch_no, self.args.save_interval) == 0
                or disp_epoch_no == self.args.num_epochs
            ):
                if self.args.save_model_checkpoint:
                    torch.save(
                        self.net.module.state_dict(),
                        os.path.join(
                            self.args.save_path, "net_e{}.ckpt".format(disp_epoch_no)
                        ),
                    )
                self.recorder.plot(path=self.args.save_path)
                save_mat(self.args.save_path, self.recorder.master_dict)
                self.recorder.master_dict_to_json(self.args.save_path)
    ...
    def train_epoch(self, epoch_no):
        disp_epoch_no = epoch_no + 1
        num_batches = int(
            np.floor(len(self.train_loader.dataset) / self.train_loader.batch_size)
        )

        disp_values = 0
        num_processed = 0
        start_time = time.time()
        for batch_no, (images, targets, masks, source_images) in enumerate(
            self.train_loader
        ):
            ...
            losses = loss_maker(
                cls_logits, attn_logits, cls_targets, attn_targets, self.config
            )
            accus, num_valid_cases = accuracy_maker(
                cls_logits, attn_logits, cls_targets, attn_targets, self.config
            )
            preds, probs, targets = prediction_maker(
                cls_logits, cls_targets, self.config
            )
            ...
            self.recorder.add_info(
                epoch_no, "tra", {"batch_size": [processed_batch_size]}
            )

            train_loss = torch.stack(list(losses.values())).sum()
            disp_values += np.array(
                [train_loss.item() * processed_batch_size]
                + [
                    v.item() * n.item()
                    for v, n in zip(losses.values(), num_valid_cases.values())
                ]
                + [
                    v.item() * n.item()
                    for v, n in zip(accus.values(), num_valid_cases.values())
                ]
            )
            disp_names = ["loss"] + list(losses.keys()) + list(accus.keys())

            num_processed += np.array(
                [processed_batch_size]
                + [n.item() for n in num_valid_cases.values()] * 2
            )
            avg_disp_values = dict(
                zip(disp_names, (disp_values / np.maximum(num_processed, 1)).tolist())
            )
            ...
            if (
                np.mod(disp_batch_no, self.args.display_interval) == 0
                or disp_batch_no == num_batches
            ):
                disp_str_tmp = "[{} Elapsed: {}] [tra] [epoch{} {}/{}] "
                disp_str_tmp += " ".join(
                    [
                        "{}: {:.3f}".format(s, avg_disp_values[s])
                        for s in avg_disp_values.keys()
                    ]
                )
                print(
                    disp_str_tmp.format(
    ...
    def val_epoch(self, epoch_no):
        ...
        disp_epoch_no = epoch_no + 1
        num_batches = int(
            np.ceil(len(self.test_loader.dataset) / self.test_loader.batch_size)
        )
        ...
        with torch.no_grad():
            for batch_no, (images, targets, masks, source_images) in enumerate(
                ...
                cls_targets, attn_targets = target_maker(
                    targets, attn_targets, self.config
                )
                losses = loss_maker(
                    cls_logits, attn_logits, cls_targets, attn_targets, self.config
                )
                ...
                self.recorder.add_info(
                    epoch_no, "val", {"batch_size": [processed_batch_size]}
                )

                val_loss = torch.stack(list(losses.values())).sum()
                disp_values += np.array(
                    [val_loss.item() * processed_batch_size]
                    + [
                        v.item() * n.item()
                        for v, n in zip(losses.values(), num_valid_cases.values())
                    ]
                ...
                avg_disp_values = dict(
                    zip(
                        disp_names,
                        (disp_values / np.maximum(num_processed, 1)).tolist(),
                    )
                )
